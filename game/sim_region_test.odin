...
test_simulate_stops_player_at_wall :: proc(t: ^testing.T) {
	...
	sim_region.high_entities[0] = HighEntity{low_entity = &player, rel_pos = V3{0.5, 0, 0}}
	sim_region.high_entities[1] = HighEntity{low_entity = &wall, rel_pos = V3{1, 0, 0}}
	sim_region.high_entity_count = 2

	simulate(&sim_region, 0.1)

	// Contact occurs when the player's pivot reaches x = 0.6; without collision
